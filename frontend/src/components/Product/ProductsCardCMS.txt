...
const ProductsCardCMS = ({product, getAllProducts}) => {
    ...
    if (product.enabled == 0) {
        return (
          <div key={product.id} className="bg-red-800 shadow-lg rounded-lg p-4">
        ...
        return (
          <div key={product.id} className="bg-gray-800 shadow-lg rounded-lg p-4">
            <h4 className="font-bold text-lg mb-2 text-white">{product.name}</h4>
            <p className="text-gray-300 mb-2">Precio: ${product.price}</p>
...
export default ProductsCardCMS
